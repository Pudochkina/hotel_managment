...
import com.example.hotel_managment.db.*;
...
public class ReservationController implements Initializable {
    public TextField search_guest_txt_fld, guest_fio_txt_fld, sum_txt_fld1;
    ...
    public Button add_new_reservation_btn, find_available_rooms_btn;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        /**
         * обработка кнопки "создать бронь"
         */
        add_new_reservation_btn.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent actionEvent) {
                if (guest_fio_txt_fld != null
                        ...
                        && chosenService != null) {
                    // Если все поля прошли валидацию, пробуем добавить бронь
                    try {
                        boolean res = DBUtilsReservation.addNewReservation(guest_id, service_id, room_id, chosenDateIn, chosenDateOut);
                        if (res) {
                            Alert alert = new Alert(Alert.AlertType.INFORMATION);
                            alert.setContentText("Бронь успешно добавлена!");
                            alert.show();
                            // Очищаем форму
                            clearForm();
    ...
    public void getItem(MouseEvent mouseEvent) {
        ...
        guest_fio_txt_fld.setText(guest_fio_column.getCellData(index).toString());

        String phone_txt_fld = (guest_phone_column.getCellData(index).toString());
        guest_id = DBUtilsGuest.getIdByGuestPhone(phone_txt_fld);
    ...
    private void updateTotalSum() {
        ...
        try {
            ...
            if (checkOutDate.isBefore(checkInDate) || checkOutDate.isEqual(checkInDate)) {
                sum_txt_fld1.setText("Ошибка в датах");
                return;
            }
            ...
            sum_txt_fld1.setText(String.format("%.2f руб.", totalSum));

        } catch (Exception e) {
            sum_txt_fld1.setText("Ошибка расчёта");
        }
    }
